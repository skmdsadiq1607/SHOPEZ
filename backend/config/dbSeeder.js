...
const autoSeed = async () => {
  try {
    ...
    if (productCount === 0) {
      console.log('Auto-Seeder: Product collection is empty. Seeding 20 products...');
      await Product.insertMany(sampleProducts);
      console.log('Auto-Seeder: Successfully seeded products!');
    }

    const adminCount = await User.countDocuments({ usertype: 'admin' });
    if (adminCount === 0) {
      console.log('Auto-Seeder: Admin user is missing. Seeding admin...');
      ...
      console.log('Auto-Seeder: Successfully seeded admin!');
    }
  } catch (err) {
